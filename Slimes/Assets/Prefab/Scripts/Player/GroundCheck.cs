...
public class GroundCheck : MonoBehaviour
{
    ...
    //Wheteher or not the player was grounded last check
    [HideInInspector]
    ...
    public bool CheckGrounded()
    {
        ...
        // Find the collider that overlap this one
        ...
        foreach(Collider2D overlapCollider in overlaps)
        {
            ...
            {
                // This line determines if the coolider found is on a layer in the ground layer mask
                int match = contactFilter.layerMask.value & (int)Mathf.Pow(2, overlapCollider.gameObject.layer);
                Debug.Log(match);
                if (match > 0)
                {
                    //Debug.Log(groundedLastCheck);
